...
def main():
    ...
    for i, caminho_pdf in enumerate(arquivos_pdf, 1):
        ...
        try:
            ...
            dados_todos_pdfs.append((caminho_pdf, dados_extraidos))

            print(f"✓ Dados extraídos de: {caminho_pdf.name}")
    ...
    if dados_todos_pdfs:
        df_consolidado = criar_dataframe_consolidado(dados_todos_pdfs)

        # Salva em Excel
        df_consolidado.to_excel(ARQUIVO_EXCEL_SAIDA, index=False, engine='openpyxl')
        print(f"✓ Arquivo Excel consolidado salvo em: {ARQUIVO_EXCEL_SAIDA}")

        # Mostra estatísticas
        print(f"Total de faturas processadas: {len(dados_todos_pdfs)}")
        print(f"Total de colunas no Excel: {len(df_consolidado.columns)}")
